...
class TradingEngine:
    ...
    def _execute_decisions(self, decisions: Dict, market_state: Dict,
        ...
        for coin, decision in decisions.items():
            ...
            try:
                # 基于 Daily RSI (MCP, 1D) 和 Intraday RSI(14) 的程序级风控过滤，只影响开多/开空，不阻止减仓和止损
                indicators = market_state.get(coin, {}).get('indicators', {}) or {}
                daily_rsi_val = indicators.get('mcp_rsi')
                intraday_rsi_val = indicators.get('rsi_14')
                rsi_block_reason = None

                # 做多方向的开仓（buy_to_enter）过滤
                if signal == 'buy_to_enter':
                    # 1) 日线强空头趋势时，不允许新开多仓
                    try:
                        if daily_rsi_val is not None:
                            dr = float(daily_rsi_val)
                            if dr <= 40:
                                rsi_block_reason = f"Daily RSI={dr:.1f} 显示强空头趋势，阻止开多"
                    except (TypeError, ValueError):
                        pass

                    # 2) 盘中 RSI(14) 过滤：只有在相对偏低（<=45）时才允许新开多单，避免在盘中高位频繁追涨
                    if rsi_block_reason is None:
                        try:
                            if intraday_rsi_val is not None:
                                ir = float(intraday_rsi_val)
                                # 当 Intraday RSI(14) 高于 45 时，认为当前价格不够“便宜”，不在这个区域开多
                                if ir > 45:
                                    rsi_block_reason = f"Intraday RSI(14)={ir:.1f} 偏高（>45），当前不是逢低做多区间，阻止开多"
                        except (TypeError, ValueError):
                            pass

                # 做空方向的开仓（sell_to_enter）过滤
                    ...
                    try:
                        if daily_rsi_val is not None:
                            dr = float(daily_rsi_val)
                            if dr >= 60:
                                rsi_block_reason = f"Daily RSI={dr:.1f} 显示强多头趋势，阻止开空"
                    except (TypeError, ValueError):
                        pass

                    # 2) 盘中 RSI(14) 过滤：只有在相对偏高（>=55）时才允许新开空单，避免在盘中低位频繁追空
                    if rsi_block_reason is None:
                        try:
                            if intraday_rsi_val is not None:
                                ir = float(intraday_rsi_val)
                                # 当 Intraday RSI(14) 低于 55 时，认为当前价格不够“贵”，不在这个区域开空
                                if ir < 55:
                                    rsi_block_reason = f"Intraday RSI(14)={ir:.1f} 偏低（<55），当前不是逢高做空区间，阻止开空"
                        except (TypeError, ValueError):
                            pass
                ...
                if rsi_block_reason:
                    result = {
                        'coin': coin,
                        'signal': 'hold',
                        'message': f'RSI 风控过滤: {rsi_block_reason}'
                    }
    ...
    def _execute_reduce_position(self, coin: str, decision: Dict, market_state: Dict,
        ...
        return {
            ...
            'pnl': pnl,
            'side': side,
            'message': f'Reduce {side} position: {quantity:.4f} {coin} @ ${current_price:.2f}, P&L: ${pnl:.2f}'
        }
