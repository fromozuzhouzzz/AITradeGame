...
class AITrader:
    def __init__(self, api_key: str, api_url: str, model_name: str, max_retries: int = 3):
        ...
        # 全局控制：避免在同一轮中为多个模型重复打印完全相同的长 PROMPT
        if not hasattr(AITrader, "_last_prompt_text"):
            AITrader._last_prompt_text = None
    ...
    def _build_prompt(self, market_state: Dict, portfolio: Dict,
        ...
        for coin, data in market_state.items():
            ...
            rsi_9_15m = indicators.get('rsi_9_15m')
...
"""
            ...
            # Safely format short-term RSI(9, 15m) with unified 35/65 thresholds
            if rsi_9_15m is not None:
                try:
                    rsi_9_val = float(rsi_9_15m)
                    rsi_9_text = f"{rsi_9_val:.1f} "
                    if rsi_9_val < 35:
                        rsi_9_text += "- OVERSOLD"
                    elif rsi_9_val > 65:
                        rsi_9_text += "- OVERBOUGHT"
                    else:
                        rsi_9_text += "- Neutral"
                except (TypeError, ValueError):
                    rsi_9_text = "N/A - Invalid value"
            else:
                rsi_9_text = "N/A - unavailable (Binance 15m data missing)"
...
MOMENTUM (30% weight):
✓ Short-term RSI(9, 15m) < 35 AND you are buying/adding LONG or covering SHORTS = +1
✓ Short-term RSI(9, 15m) > 65 AND you are selling/adding SHORT or taking profit on LONGS = +1
...
1. **TRENDING MARKET** (Clear directional moves)
   ...
   - Strategy: Trend following with 1x leverage (spot only)
...
2. **RANGING MARKET** (Consolidation phase)
   ...
   - Strategy: Mean reversion with 1x leverage (spot only)
...
3. **VOLATILE/BREAKOUT MARKET** 
   ...
   - Strategy: Wait for confirmation with 1x leverage (spot only)
...
- High Confidence (0.8+):
  ...
  * 杠杆: 1x (spot only, no additional leverage)
  * 目标保证金: $8,000-$12,000 (在1x杠杆下等同于实际投入资金)
  * 示例: BNB $100K, 使用1x杠杆, 目标保证金$10K → quantity = $10K / 当前价格

- Medium Confidence (0.6-0.7):
  * 风险: 5-7% ($5,000-$7,000)
  * 杠杆: 1x (spot only, no additional leverage)
  * 目标保证金: $5,000-$8,000 (在1x杠杆下等同于实际投入资金)
  * 示例: ETH $3.6K, 使用1x杠杆, 目标保证金$7K → quantity = $7K / $3.6K

- Low Confidence (0.4-0.5):
  * 风险: 3-5% ($3,000-$5,000)
  * 杠杆: 1x (spot only, no additional leverage)
  * 目标保证金: $3,000-$5,000 (在1x杠杆下等同于实际投入资金)
  * 示例: SOL $200, 使用1x杠杆, 目标保证金$5K → quantity = $5K / $200
...
1. RSI(9, 15m) < 35 + Price > SMA14 + EMA12 > EMA26 + MACD turning up
...
1. RSI(9, 15m) > 65 + Price < SMA14 + EMA12 < EMA26 + MACD turning down
...
- Maintain at least 20% cash for new opportunities and risk control
...
5. Leverage requirements:
   - All trades must use 1x leverage (spot-only execution environment).
   - You MUST always set the "leverage" field to 1 for every coin, and compute quantity under this assumption.
...
"""  
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        """
        # Debug logging: 如果本次要发送的 PROMPT 与上一次完全相同，则不重复打印
        if getattr(AITrader, "_last_prompt_text", None) != prompt:
            AITrader._last_prompt_text = prompt
            print("[LLM DEBUG] ==== 提交给大模型的 PROMPT 开始 ====")
            print(prompt)
            print("[LLM DEBUG] ==== 提交给大模型的 PROMPT 结束 ====")
...
- 分散投资: 同时持有4-6个不同币种的头寸
   - 现金储备: 保持至少20%的可用现金用于加仓和新机会
- 杠杆控制: 当前环境严格不使用杠杆，所有交易按现货1倍执行 (leverage 始终为1)
...
### 限制条件
- 头寸限制: 单个币种名义价值不超过总资金的40%，单笔名义资金建议不超过总资金的10%（现货，无保证金概念）
- 杠杆限制: leverage 字段必须固定为1，禁止使用大于1的杠杆
- 风险回报: 目标风险回报比不低于2:1
- 确认要求: 至少3个技术指标确认交易信号
...
Output JSON format only."""
            ...
            # Debug logging: record raw model response for this prompt
            print("[LLM DEBUG] ==== 大模型原始返回 BEGIN ====")
            print(raw_content)
            print("[LLM DEBUG] ==== 大模型原始返回 END ====")
