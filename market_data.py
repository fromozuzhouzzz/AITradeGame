...
class MarketDataFetcher:
    ...
    def calculate_technical_indicators(self, coin: str) -> Dict:
        ...
        """
        # Try to get from persistent cache first (max age: 60 minutes)
        cached_indicators = None
        if self.persistent_cache:
            cached_indicators = self.persistent_cache.get_technical_indicators(coin, max_age_minutes=60)
            if cached_indicators:
                self._cache_hits += 1
                print(f"[CACHE HIT] Using cached technical indicators for {coin}")
                # Don't return yet - we still want to enhance with MCP data
                indicators = cached_indicators.copy()
            else:
                self._cache_misses += 1
                print(f"[CACHE MISS] Calculating technical indicators for {coin}")
                cached_indicators = None

        # Only calculate if cache miss or disabled
        if not cached_indicators:
            # Cache miss or disabled - calculate from historical data
        ...
        if self.mcp_client:
            print(f"[MCP DEBUG] Attempting to enhance indicators for {coin}...")
            try:
                # 1. 获取 K 线数据和技术指标
                ...
                if mcp_indicators:
                    ...
                    print(f"[MCP] ✓ Enhanced indicators for {coin}: {len(parsed_fields)} fields ({', '.join(parsed_fields[:5])}...)")
                else:
                    print(f"[MCP] ✗ No MCP indicators returned for {coin} (parsing may have failed)")

                # 2. 获取借贷比率
                loan_ratios = self.mcp_client.get_okx_loan_ratios(coin)
                if loan_ratios:
                    indicators['mcp_loan_ratios'] = loan_ratios
                    print(f"[MCP] OK Retrieved loan ratios for {coin}")

                # 3. 获取主动买卖交易量
                taker_volume = self.mcp_client.get_okx_taker_volume(coin)
                if taker_volume:
                    indicators['mcp_taker_volume'] = taker_volume
                    print(f"[MCP] OK Retrieved taker volume for {coin}")

                # 4. 获取币安 AI 分析报告
                ai_report = self.mcp_client.get_binance_ai_report(coin)
                if ai_report:
                    indicators['mcp_binance_ai_report'] = ai_report
                    print(f"[MCP] OK Retrieved Binance AI report for {coin}")

            except Exception as e:
                print(f"[MCP WARNING] Failed to enhance indicators with MCP: {e}")
                import traceback
                traceback.print_exc()
        else:
            print(f"[MCP DEBUG] MCP client not available, skipping indicator enhancement for {coin}")
    ...
    def get_crypto_news(self, coin: str, limit: int = 5) -> List[Dict]:
        ...
        if not self.mcp_client:
            print("[WARNING] MCP client not available, cannot fetch news")
            return []
        ...
        try:
            ...
            print(f"[ERROR] Failed to fetch news for {coin}: {e}")
            return []
